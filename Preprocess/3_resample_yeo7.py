...
def registrationJob(sub):
    ...
    t1_2_mni.inputs.moving_image = '/data_1mm/' + sub + '/fs/mri/brain.nii.gz'  # Freesurfer生成的brain.mgz,
    t1_2_mni.inputs.moving_mask = '/data_1mm/' + sub + '/fs/mri/brainmask.nii.gz'  # Freesurfer生成的brainmask.mgz,
    ...
    get_atlas.inputs.reference_image='/data_1mm/'+sub+'/fs/mri/brain.nii.gz' # 1mm的brain.nii.gz

    ramdom_str=random.sample('abcdefghijklmnopqrestuvwxyz0123456789',8)
    work_dir='/data_1mm/tmp%s' % ''.join(ramdom_str)
